...
import { Calendar, MapPin, Ticket, CheckCircle, Download, XCircle } from "lucide-react";
...
export function MyTickets() {
  ...
  const handleCancel = async (ticketId: string, eventTitle: string) => {
    if (!user) return;
    setCancellingId(ticketId);
    try {
      await ticketApi.cancel(ticketId, user.id);
      toast.success(`Ticket for "${eventTitle}" has been cancelled.`);
              ...
              <div key={eventId} className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div className="p-4 md:p-6 space-y-3">
                  ...
                  {tickets.map((ticket, index) => {
                    ...
                    return (
                      <div key={ticket.id} className={`flex items-center justify-between p-4 rounded-lg border transition-colors ${isCancelled
                          ? "bg-red-50 border-red-200 opacity-75"
                          : "bg-gray-50 border-gray-200 hover:border-indigo-200 hover:bg-indigo-50/30"
                        }`}>
                        <div className="flex items-center space-x-4">
                          <div className={`w-10 h-10 rounded-full flex items-center justify-center ${isCancelled ? "bg-red-100" : "bg-indigo-100"
                            }`}>
                            {isCancelled
                          ...
                          <div>
                            ...
                            {isCancelled && (
                              <span className="inline-block mt-1 px-2 py-0.5 rounded-full bg-red-100 text-red-700 text-xs font-semibold">
                                Cancelled
                              </span>
                            )}
                          </div>
                          ...
                          {!isCancelled && (
                            <>
                              ...
                              {confirmCancelId === ticket.id ? (
                                <div className="flex items-center gap-2">
                                  <span className="text-xs text-red-600 font-medium hidden sm:inline">Are you sure?</span>
                                  <button
                                    onClick={() => handleCancel(ticket.id, event.title)}
                                    disabled={cancellingId === ticket.id}
                                    className="flex items-center px-3 py-2 bg-red-600 text-white rounded-lg text-xs font-medium hover:bg-red-700 transition-colors shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
                                  >
                                ...
                                <button
                                  ...
                                  className="flex items-center px-3 py-2 bg-red-50 text-red-600 rounded-lg text-sm font-medium hover:bg-red-100 transition-colors border border-red-200"
                                >
